    ...
    returns (uint256)
{
    if (block.timestamp < schedule.startTime.add(schedule.cliffDuration)) {
        return 0;
    } else if (block.timestamp >= schedule.startTime.add(schedule.vestingDuration)) {
        return schedule.totalAmount.sub(schedule.claimedAmount);
    } else {
        uint256 elapsedTime = block.timestamp.sub(schedule.startTime.add(schedule.cliffDuration));
        uint256 vestingPeriod = schedule.vestingDuration.sub(schedule.cliffDuration);
        uint256 vestedAmount = schedule.totalAmount.mul(elapsedTime).div(vestingPeriod);
